...
plot_caption <- 'source: data retrieved from Google Places API'
```
...
ggplot(popglob, aes(x = local_time, y = diff_mean)) +
    ...
    xlab(NULL) + ylab(NULL) +
    labs(title = 'Mean difference in popularity score over time',
         subtitle = plot_subtitle,
         caption = plot_caption) +
    theme_minimal()
```
...
```{r}
poplocality <- group_by(popsm, local_time, category) %>%
...
ggplot(filter(poplocality, !is.na(diff_mean)), aes(x = category, y = diff_mean)) +
    ...
    xlab(NULL) +
    ylab('Difference') +
    labs(title = 'Difference in popularity score per place category',
         subtitle = plot_subtitle,
         caption = plot_caption) +
    theme_minimal()
```
...
ggplot(poplocality, aes(x = local_time, y = diff_mean, color = category)) +
    ...
    xlab(NULL) + ylab(NULL) +
    labs(title = 'Mean difference in popularity score per category over time',
         subtitle = plot_subtitle,
         caption = plot_caption) +
...
ggplot(popcountry_filt, aes(x = country, y = diff_mean)) +
    geom_hline(yintercept = 0, linetype = 3) +
    geom_boxplot(outlier.shape = NA) +
    geom_jitter(width = 0.1, alpha = 0.3) +
...
ggplot(popcountry_cats_filt, aes(x = reorder(country, desc(country)), y = diff_mean)) +
    geom_hline(yintercept = 0, linetype = 3) +
    geom_boxplot(outlier.shape = NA) +
