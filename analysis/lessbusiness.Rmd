...
```{r}
qplot(popsm$pop_diff, binwidth = 10) + scale_x_continuous(limits = c(-100, 200)) + theme_minimal()
```
...
ggplot(popcountry_cats_filt, aes(x = country, y = diff_mean)) +
    geom_boxplot() +
    geom_hline(yintercept = 0, linetype = 3) +
    scale_x_discrete(limits = rev(sort(unique(popcountry_cats_filt$country)))) +
    coord_flip() +
