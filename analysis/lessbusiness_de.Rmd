...
pop <- select(pop, -utc_date, -utc_hour, -lat, -lng) # %>% filter(pop_frac <= 5)

pop_approx <- select(pop, -current_pop, -usual_pop) %>% group_by(local_day, place_id) %>%
...
cntrypop <- read.csv('../data/countries_population.csv', stringsAsFactors = FALSE) %>%
    filter(iso_a2 %in% unique(popeur$iso2)) %>%
    rename(population = pop_est)
#cntrypop <- mutate(cntrypop, country_weight = pop_est / sum(pop_est))
#stopifnot(sum(cntrypop$country_weight) == 1)

country_cat_means <- group_by(popeur, iso2, category) %>% group_means_ci(pop_diff) %>% ungroup()
country_cat_means <- left_join(select(cntrypop, iso_a2, population), country_cat_means, by = c('iso_a2' = 'iso2'))
country_cat_means <- group_by(country_cat_means, category) %>% mutate(country_cat_weight = population / sum(population)) %>% ungroup()

stopifnot(all(group_by(country_cat_means, category) %>% summarise(sum_check = sum(country_cat_weight)) %>% pull(sum_check) == 1))

cat_means <- group_by(country_cat_means, category) %>% summarise(weighted_mean_pop_diff = sum(country_cat_weight * mean_pop_diff),
                                                                 unweighted_mean_pop_diff = mean(mean_pop_diff)) %>% ungroup()
...
plot_means_hbars(cat_means, category, weighted_mean_pop_diff, 'Differences in popularity score per category in Europe', collection_time)
```
...
#daily_country_cat_max_diff <- group_by(popdaily, country, category, local_day) %>%
#    summarise(day_max_diff = max(abs(pop_diff), na.rm = TRUE)) %>% ungroup()

# daily_place_absdiffsum <- group_by(popdaily, place_id, local_day) %>%
#     summarise(day_absdiffsum = sum(abs(pop_diff), na.rm = TRUE)) %>% ungroup()
# 
# 
# popdaily_w_absdiffsum <- left_join(popdaily, daily_place_absdiffsum, by = c('place_id', 'local_day')) %>%
#     mutate(rel_pop_diff = pop_diff / day_absdiffsum)
# stopifnot(sum(is.na(popdaily_w_absdiffsum$day_absdiffsum)) == 0)
# 
# daily_country_cat_means <- group_by(popdaily_w_absdiffsum, country, category, local_weekend, local_hour) %>%
#     group_means_ci(rel_pop_diff) %>% ungroup()
# 
# 
# daily_cat_means <- group_by(daily_country_cat_means, category, local_weekend, local_hour) %>%
#     group_means_ci(mean_rel_pop_diff) %>% ungroup() %>% filter(n_mean_rel_pop_diff > 1)
