...
```{r, fig.width=10, fig.height=5}
popcntrs <- filter(pop, country %in% c('Italy', 'Spain', 'Germany', 'Sweden', 'United States', 'Taiwan', 'Korea, South'),
                   !(category %in% c('hardware stores')))
ts_cntrs_cat_daily_means <- group_by(popcntrs, local_day, country, category) %>% group_means_ci(pop_diff) %>% ungroup()

ts_cntrs_daily_means <- group_by(ts_cntrs_cat_daily_means, local_day, country) %>% group_means_ci(mean_pop_diff) %>% ungroup()


plot_country_daily_means(ts_cntrs_daily_means, local_day, mean_mean_pop_diff, lwr_mean_pop_diff, upr_mean_pop_diff, country,
...
cities_means <- left_join(cities_means, n_places_per_city, by = c('iso2', 'city')) %>%
    left_join(cities, by = c('iso2', 'city')) %>% filter(n_places_per_city >= 5)

plot_cities_map(cities_means, 'Mean popularity difference per city', collection_time)
```
